...
3.) Change a value, e.g. the date to another date in the current month or amount (date=06.05.2025, amount=5.00 and type=Supermarket)
    -> checks that only correct dates (workday, within the current month and not already uploaded an image on that day) and amount are accepted (not minus and a number)

4.) Go to "eingerechte Rechnungen"
5.) In that table, you should see the uploaded invoice (refund=3.00, status=PENDING, typ date=06.05.2025 and user=yourUserName) 
